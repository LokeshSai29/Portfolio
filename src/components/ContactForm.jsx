...
const ContactForm = () => {
        ...
        if (Object.keys(validationErrors).length > 0) {
            ...
            emailjs
                ...
                .catch((error) => {
                    ...
                    toast.error("Failed to sent message.Please try agaiin");
                })
                .finally(()=>{
                    setIsSending(false);
                })
  ...
  return (
    <div className="p-4 lg:w-3/4" id="contact">
        <Toaster/>
        <h2 className="text-center text-4xl lg:text-4xl tracking-tighter font-mono">Let's Connect</h2>
        <motion.form
            initial={{opacity :0}}
        ...
        onSubmit={handleSubmit}>
                <div className="flex space-x-4 py-8">
                    ...
                    <div className="lg:w-1/2">
                        ...
                        className="mb-4 w-full appearance-none rounded-lg border border-stone-50/30 bg-transparent px-3 py-4 text-sm 
                        focus:border-stone-400 focus:outline-none" />
                        {errors.name &&(
                            <motion.p 
                                initial={{opacity :0}}
                                whileInView={{opacity: 1}}
                            className="text-sm text-rose-700"> {errors.name}</motion.p>
                        )}    
                    </div>
                    <div className="lg:w-1/2">
                        <input type="email" id="email" name="email" value={formData.email} placeholder="Email" onChange={handleChange} 
                        className="mb-8 w-full appearance-none rounded-lg border border-stone-50/30 bg-transparent px-3 py-4 text-sm 
                        focus:border-stone-400 focus:outline-none" />
                        {errors.email &&(
                            <motion.p 
                            initial={{opacity :0}}
                            whileInView={{opacity: 1}} className="text-sm text-rose-700"> {errors.email}</motion.p>
                        )}    
                    </div>
                ...
                <div className="mt-0 mb-2">
                        ...
                        {errors.message &&(
                            <motion.p 
                            initial={{opacity :0}}
                            whileInView={{opacity: 1}} className="text-sm text-rose-700"> {errors.message}</motion.p>
                        )}    
                </div>
                <button type="submit" className={`mb-8 w-full  rounded border 
                        border-stone-50/30 bg-stone-200 px-4 py-2 text-lg
